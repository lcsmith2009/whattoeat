(() => {
  ...
  Object.entries(jsonDefaults).forEach(([key, fallback]) => {
    ...
    try {
      ...
      if (!validShape) localStorage.setItem(key, JSON.stringify(fallback));
    } catch (error) {
  ...
  if (streakRaw !== null) {
    ...
    if (!Number.isFinite(streak) || streak < 0) localStorage.setItem('wte_streak', '1');
  }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const observer = new MutationObserver(() => {
      if (isOpen()) {
        if (document.activeElement && !modal.contains(document.activeElement)) previouslyFocused = document.activeElement;
